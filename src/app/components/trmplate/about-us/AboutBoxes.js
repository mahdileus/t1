            ...
            <div className="pt-44">
                <div className="flex flex-col gap-8 items-center justify-center">
                    <h2 className="text-center text-4xl font-bold text-primary w-full md:w-[40%]">
                        خدمات <span className="text-secondery">تیوان</span> – از ایده تا موفقیت آنلاین
                    </h2>
                    <h4 className="text-lg justify-last-center w-full md:w-[40%]">ما در تیوان با ارائه مجموعه‌ای کامل از خدمات دیجیتال مارکتینگ، از طراحی وبسایت اختصاصی و برنامه‌نویسی پیشرفته گرفته تا سئو، تبلیغات و افزایش فروش، همراه شما هستیم تا کسب‌وکارتان را به شکلی حرفه‌ای در فضای آنلاین رشد دهید. </h4>
                </div>
                <div className="relative ">


                    {/* باکس‌ها */}
                    <div className="container grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-14 mt-14 relative z-10">
                        {services.map((item, index) => (
                            <div
                                key={index}
                                className="border hover:border-[#ffcda6] hover:-translate-y-12 transition-all border-gray-100 relative px-8 pt-18 pb-8 rounded-4xl my-4 h-[350px] bg-white"
                            >
                                <span className="absolute -top-12 right-1/3 transition-all bg-[#fff3f0] p-8 rounded-full">
                                    {item.icon}
                                </span>
                                <h3 className="font-yekan-bakh text-center font-medium text-lg mt-2 text-primary">
                                    {item.title}
                                </h3>
                                <p className="text-gray-600 p-4 text-sm rounded-3xl bg-gray-100 mt-4 font-yekan-bakh text-justify">
                                    {item.desc}
                                </p>
                            </div>
                        ))}
